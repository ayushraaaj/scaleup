...
export const getMentorPosts = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const posts = await Post.find({ mentorId: userId })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .populate("mentorId", "fullname username");

    const totalPosts = await Post.countDocuments({ mentorId: userId });

    return res.status(200).json(
      new ApiResponse(`Post fetched`, {
        page,
        totalPages: Math.ceil(totalPosts / limit),
        totalPosts,
        posts,
...
export const reactToPost = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!existingReaction) {
    await Reaction.create({ postId: post._id, userId, type });
...
export const deletePost = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(200).json(new ApiResponse("Post deleted successfully", {}));
});
...
export const editPost = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse("Post details updated successfully", updatedPost));
});
